...
// are, in full:
//
//  1. Added the `System` and `System.Collections.Generic` using directives below, to
//     compile against this project's net48 target (the source file relies on implicit
//     usings, which this project does not enable project-wide, to avoid an unrelated
//     UnityEngine.Object/object clash elsewhere in the mod).
//
//  2. Hoisted the Rx/Rz/Crx trigonometry out of the per-amplitude inner loops in
//     Simulator.ApplySingle and Simulator.ApplyControlled. Turn() and Phaseturn() are
//     called once per amplitude PAIR, and each call re-evaluated Math.Cos/Math.Sin (or
//     Complex.FromPolarCoordinates) for a theta that is fixed for the whole gate. For
//     the 16-qubit rotation layer this mod runs per colour channel that was ~1 million
//     redundant transcendental calls, and it dominated the runtime. The originals are
//     kept as thin wrappers so behaviour and the public surface are unchanged.
//     This is worth reporting upstream - it is a straight win for any large circuit.
//
// All quantum behaviour is otherwise untouched; the modifications are pure hoisting.
using System;
...
public static class Simulator
{
    ...
    static (Complex, Complex) Turn(Complex a, Complex b, double theta)
        => Turn(a, b, Math.Cos(theta * 0.5), Math.Sin(theta * 0.5));

    // MODIFIED (see file header): variant taking the already-evaluated cos/sin, so the
    // callers below can compute them once per gate instead of once per amplitude pair.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static (Complex, Complex) Turn(Complex a, Complex b, double cos, double sin)
    {
    ...
    static (Complex, Complex) Phaseturn(Complex a, Complex b, double theta)
        => Phaseturn(a, b, Complex.FromPolarCoordinates(1.0, theta * 0.5));

    // MODIFIED (see file header): as with Turn, this variant takes the already-evaluated
    // phase so it can be hoisted out of the per-amplitude loop.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static (Complex, Complex) Phaseturn(Complex a, Complex b, Complex phase)
        => (a * Complex.Conjugate(phase),
            b * phase);
    ...
    static void ApplySingle(Complex[] amp, int n, in Gate g)
    {
        ...
        // MODIFIED (see file header): the Rx/Rz trigonometry depends only on the gate,
        // but used to be re-evaluated inside Turn/Phaseturn for every amplitude pair.
        // Hoisting it here removes ~2 million Sin/Cos calls per 16-qubit rotation layer.
        double cos = 0, sin = 0;
        Complex phase = default;
        if (g.Op == GateOp.Rx) { cos = Math.Cos(g.Theta * 0.5); sin = Math.Sin(g.Theta * 0.5); }
        else if (g.Op == GateOp.Rz) { phase = Complex.FromPolarCoordinates(1.0, g.Theta * 0.5); }

        int highCount = 1 << (n - target - 1);
        for (int high = 0; high < highCount; high++)
            for (int low = 0; low < step; low++)
            {
                ...
                (amp[i0], amp[i1]) = g.Op switch
                {
                    ...
                    GateOp.Rx => Turn(amp[i0], amp[i1], cos, sin),
                    _         => Phaseturn(amp[i0], amp[i1], phase), // Rz gate
                };
            }
    }
    ...
    static void ApplyControlled(Complex[] amp, int n, in Gate g)
    {
        ...
        // MODIFIED (see file header): same hoist as in ApplySingle, for Crx.
        double cos = 0, sin = 0;
        if (g.Op == GateOp.Crx) { cos = Math.Cos(g.Theta * 0.5); sin = Math.Sin(g.Theta * 0.5); }
            ...
            for (int b = 0; b < (1 << (hi - lo - 1)); b++)
                for (int c = 0; c < (1 << (n - hi - 1)); c++)
                {
                    ...
                    switch (g.Op)
                    {
                        ...
                        case GateOp.Crx:  (amp[b10], amp[b11]) = Turn(amp[b10], amp[b11], cos, sin); break;
